...
interface UserCredits {
  ...
  remaining_audio_credits: number;
...
export const useCredits = (userId: string | undefined, planId: string | undefined) => {
    ...
    if (!userId || hasUnlimitedCredits) {
      ...
      if (hasUnlimitedCredits) {
        setCredits({
          ...
          used_credits: 0,
          remaining_credits: -1,
          total_audio_credits: -1,
          used_audio_credits: 0,
          remaining_audio_credits: -1,
          trial_ends_at: null,
    ...
    try {
      ...
      if (data) {
        ...
        setCredits({
          ...
          total_audio_credits: data.total_audio_credits || 14,
          used_audio_credits: data.used_audio_credits || 0,
          remaining_audio_credits: Math.max(0, (data.total_audio_credits || 14) - (data.used_audio_credits || 0)),
        ...
        if (newData) {
          ...
          setCredits({
            ...
            total_audio_credits: newData.total_audio_credits || 14,
            used_audio_credits: newData.used_audio_credits || 0,
            remaining_audio_credits: (newData.total_audio_credits || 14) - (newData.used_audio_credits || 0),
            trial_ends_at: trialEndsAt,
            is_trial_expired: false,
          });
        }
  ...
  const useAudioCredit = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setCredits(prev => prev ? {
        ...
        used_credits: prev.used_credits + 1,
        remaining_credits: prev.remaining_credits - 1,
        used_audio_credits: prev.used_audio_credits + 1,
  ...
  const canSendAudio = useCallback((): boolean => {
    if (hasUnlimitedCredits) return true;
    if (!credits) return false;
    return credits.remaining_audio_credits > 0 && credits.remaining_credits > 0 && !credits.is_trial_expired;
  }, [credits, hasUnlimitedCredits]);
